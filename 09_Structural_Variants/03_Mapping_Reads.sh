...
DATA_DIR=/data/scratch/btx316/Comp_Genomics/01_Trimmed_Reads/QC2
OUTDIR=/data/scratch/btx316/Comp_Genomics/02_Mapped/"$Genome"
...
## Getting the R1 read
R1_file=`sed $ID"q;d" $SCRIPTS/03_R1_File_List.txt`
echo $R1_file
...
newname=`basename $R1_file | sed -e "s/.read_1.clean_trimmed.fastq.gz/_"${Genome,,}"_aligned.sam/"`
echo $newname
...
RG_LB=`echo $RG_PU | sed -e "s/.read_1/"${Genome,,}".lib/"`
echo "@RG\tID:$Population$RG_ID\tPU:$RG_PU\tSM:$RG_SM\tLB:$RG_LB\tPL:Illumina"

bwa mem -M -t 2 -R "@RG\tID:$Population$RG_ID\tPU:$RG_PU\tSM:$RG_SM\tLB:$RG_LB\tPL:Illumina" $REFERENCE/"$Genome"_Reference_Masked.fasta $DATA_DIR/"$R1_file" $DATA_DIR/"$R2_file" > $OUTDIR/sam_files/"$newname"

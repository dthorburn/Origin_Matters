...
#DATA_DIR=/data/archive/archive-SBCS-EizaguirreLab/Miles/Local_Reference_Genome/02_Trimmed_fastq
...
## Getting the R1 read
R1_file=`sed ${SGE_TASK_ID}"q;d" $DATA_DIR/01_R1_File_List.txt`
echo $R1_file
...
bwa mem -M -t 2 -R "@RG\tID:$Population$RG_ID\tPU:$RG_PU\tSM:$RG_SM\tLB:$RG_LB\tPL:Illumina" $REFERENCE/Gynogen_pchrom_assembly_all.fasta $DATA_DIR/"$R1_file" $DATA_DIR/"$R2_file" > $OUTDIR/sam_files/"$newname"

######################################################################################
###                PopGenome Local Reference Genome Scans                          ###
######################################################################################
## Date: 22/06/2020
## Involved: Miles
## Task: Scan the SNP calls based on the two alternative reference genomes across population level, and calculate Fst at the pair level
...
for(level in c("population", "population_pair")[2]){
	if(level == "population"){
		## Population level
		pop_names <- list(	G1_R <- c(paste0("BS",seq(1,11,2))), 
		...
		pop_names <- list(	G1 <- c(paste0("BS", seq(1,12,1))), 
...
print(4)
			...
			if(chrom == 22){
				fwrite(output, file = paste("/data/scratch/btx316/Comp_Genomics/12_PopGenome_Scan/01_Raw_Results/All", origin, population, level, "scan_wDXY_20kb.csv", sep = "_"), row.names = FALSE)
